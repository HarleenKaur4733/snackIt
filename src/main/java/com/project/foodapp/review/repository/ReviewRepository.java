package com.project.foodapp.review.repository;
...
public interface ReviewRepository extends JpaRepository<Review, Long> {
    ...
    @Query("SELECT CASE WHEN COUNT(r) > 0 THEN true ELSE false END FROM Review r WHERE r.user.id = :userId AND r.menu.id = :menuId AND r.order.id = :orderId")
